(() => {
  ...
  const SLOTS = ["hhg-slot-1", "hhg-slot-2", "hhg-slot-3"];
  ...
  const ART = {
    "dont-panic": { src: "art/dont-panic.jpg", cap: "COVER / DON'T PANIC" },
    bedroom: { src: "art/bedroom.jpg", cap: "LOCAL / BEDROOM / EARTH" },
    house: { src: "art/earth-house.jpg", cap: "LOCAL / FRONT OF HOUSE" },
    lane: { src: "art/lane.jpg", cap: "LOCAL / COUNTRY LANE" },
    pub: { src: "art/pub.jpg", cap: "LOCAL / PUB / PEANUTS AND BEER" },
    earth: { src: "art/earth.jpg", cap: "PLANET / EARTH / MOSTLY HARMLESS" },
    space: { src: "art/space.jpg", cap: "DEEP SPACE / IN TRANSIT" },
    vogon: { src: "art/vogon.jpg", cap: "VOGON CONSTRUCTOR FLEET" },
    hog: { src: "art/heart-of-gold.jpg", cap: "STARSHIP / HEART OF GOLD" },
    magrathea: { src: "art/magrathea.jpg", cap: "PLANET / MAGRATHEA" },
    guide: { src: "art/guide-open.jpg", cap: "DEVICE / THE GUIDE" },
  };
  ...
  const NOUNS = [
    "gown", "dressing gown", "pocket", "screwdriver", "toothbrush", "tablet",
    "aspirin", "phone", "bed", "curtain", "window", "light", "bulldozer",
    "prosser", "ford", "towel", "satchel", "thumb", "sandwich", "beer",
    "peanuts", "babel fish", "dispenser", "panel", "hook", "mail", "fluff",
    "tea", "cup", "machine", "keypad", "marvin", "guide", "sachet", "junk mail",
  ];
  ...
  function plate(key) {
    const cap = (ART[key] && ART[key].cap) || key;
    ...
    g.fillText("LOCATION PLATE / " + key.toUpperCase(), 56, 540);
    return c.toDataURL("image/png");
  }

  function artSrc(key) {
    if (window.ART_DATA && window.ART_DATA[key]) return window.ART_DATA[key];
    return plate(key);
  }

  function setArt(key) {
    if (!ART[key] || state.lastArt === key) return;
    state.lastArt = key;
    const img = $("#art");
    img.classList.add("dim");
    setTimeout(() => {
      img.src = artSrc(key) || ART[key].src;
      $("#artCap").textContent = ART[key].cap;
      img.classList.remove("dim");
    }, 160);
  }

  function gameText() {
    return ($("#windowport") && $("#windowport").innerText) || "";
  }

  function statusText() {
    return ($("#statusline") && $("#statusline").textContent) || "";
  }
  ...
  function scanWorld() {
    ...
    for (const rule of ART_RULES) {
      if (rule.test.test(tail)) { setArt(rule.key); break; }
    }
    renderHints(tail);
    renderNouns(tail);
    checkDeath(tail);
  ...
  function renderHints(tail) {
    const box = $("#hints");
    box.innerHTML = "";
    if (!state.easy) { box.style.display = "none"; return; }
    ...
    for (const h of HINTS) if (h.test.test(tail)) h.cmds.forEach(add);
    if (!seen.size) ["look", "inventory", "wait"].forEach(add);
  }

  function renderNouns(tail) {
    const box = $("#nouns");
    if (!box) return;
    box.innerHTML = "";
    const low = tail.toLowerCase();
    NOUNS.filter((n) => low.includes(n)).slice(0, 10).forEach((n) => {
      const b = document.createElement("button");
      b.className = "topic noun";
      b.textContent = n;
      b.addEventListener("click", () => {
        const field = $("#command");
        const cur = field.value.trim();
        field.value = cur ? cur + " " + n : "examine " + n;
        field.focus();
      });
      box.appendChild(b);
    });
  ...
  function checkDeath(tail) {
    const dead = /you are dead|you have died|better luck next life|you black out|\*\*\* you have died/i.test(tail)
      || /\byou die\b/i.test(tail);
    const pane = $("#deadpane");
  ...
  function refreshSlots() {
    $$("[data-slot]").forEach((btn) => {
      ...
      if (mark) mark.textContent = slotLabel(key);
    });
    const auto = $("#autoMark");
  ...
  function openSlots(mode) {
    state.fileMode = mode;
    $("#slotTitle").textContent = mode === "save" ? "Save to a slot" : "Restore a slot";
  ...
  function closeSlots() {
    ...
    if (state.fileMode === "prompt-save" || state.fileMode === "prompt-restore") {
      MiniGlk.cancelFilePrompt();
    }
    state.fileMode = null;
  }
  ...
  function useSlot(key) {
    ...
    $("#slotmodal").classList.remove("show");
    if (mode === "prompt-save" || mode === "prompt-restore") {
      const existing = (mode === "prompt-restore") ? localStorage.getItem(key) : null;
      if (mode === "prompt-restore" && !existing) {
        toast("That slot is empty.");
        MiniGlk.cancelFilePrompt();
        state.fileMode = null;
        return;
      }
      MiniGlk.finishFilePrompt(key, existing);
      toast(mode === "prompt-save" ? "Saved." : "Restored.");
      state.fileMode = null;
      state.dead = false;
      $("#deadpane").classList.remove("show");
      return;
    }
    if (mode === "save") {
      const snap = MiniGlk.snapshot();
      if (!snap) { toast("Could not snapshot the universe."); return; }
      localStorage.setItem(key, snap);
      toast("Saved.");
    } else if (mode === "restore") {
      const raw = localStorage.getItem(key);
      if (!raw) { toast("Empty slot."); return; }
      const ok = MiniGlk.restoreSnapshot(raw);
      toast(ok ? "Restored. Look around." : "Restore failed.");
      if (ok) {
        MiniGlk._write(MiniGlk.current, "\n[The universe hiccups back into a previous shape.]\n>", "cmd");
        state.dead = false;
        $("#deadpane").classList.remove("show");
        submit("look");
      }
  ...
  function autosave() {
    try {
      const snap = MiniGlk.snapshot();
      if (snap) localStorage.setItem(AUTO, snap);
    } catch (e) {}
  }
  ...
  function submit(command) {
    ...
    MiniGlk.acceptLine(cmd);
    setTimeout(scanWorld, 40);
    setTimeout(scanWorld, 350);
  }
  ...
  function showTopic(name) {
    ...
    if (!body) return;
    body.innerHTML = "<h3>" + name + "</h3><p>" + (TOPICS[name] || "The Guide has much to say, little of it helpful.") + "</p>";
    setArt("guide");
  }
  ...
  function bindKeys() {
    $$("[data-cmd]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (cmd === "ANY") {
          const gag = ["look", "inventory", "wait", "enjoy life", "don't panic", "hello", "diagnose"];
          submit(gag[Math.floor(Math.random() * gag.length)]);
          return;
        }
        if (cmd === "save") { openSlots("save"); return; }
        if (cmd === "restore") { openSlots("restore"); return; }
        if (mode === "insert") typeInto(cmd);
        else submit(cmd);
      });
    });
    ...
    field.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        ...
        field.value = state.history[state.histIdx];
    ...
    $("#easyBtn").addEventListener("click", () => {
      ...
      renderHints(gameText().slice(-1800));
    });
    ...
    Object.keys(TOPICS).forEach((topic) => {
      ...
      b.addEventListener("click", () => {
        showTopic(topic);
        submit("consult guide about " + topic);
      });
      $("#topics").appendChild(b);
    });

    $$("[data-slot]").forEach((btn) => {
      btn.addEventListener("click", () => useSlot(btn.getAttribute("data-slot")));
    });
    $("#slotCancel").addEventListener("click", closeSlots);
    $("#deadRestore").addEventListener("click", () => {
      const raw = localStorage.getItem(AUTO);
      if (!raw) { toast("No autosave yet."); return; }
      const ok = MiniGlk.restoreSnapshot(raw);
      toast(ok ? "Yanked back from the wreckage." : "Autosave would not load.");
      if (ok) {
        state.dead = false;
        $("#deadpane").classList.remove("show");
        MiniGlk._write(MiniGlk.current, "\n[Don't Panic.]\n", "cmd");
  ...
  async function startMachine() {
    ...
    if (window.STORY_Z3_B64) {
      ...
      const res = await fetch("story/s4.z3");
      if (!res.ok) throw new Error("Could not load story file");
    ...
    Glk.onAfterTurn = () => { autosave(); scanWorld(); };
    Glk.onFilePrompt = (data) => {
      const func = data && data.func;
      openSlots(func === "restore" ? "prompt-restore" : "prompt-save");
    };
    vm.prepare(buf, { Glk: Glk });
    vm.init();
    state.ready = true;
    scanWorld();
